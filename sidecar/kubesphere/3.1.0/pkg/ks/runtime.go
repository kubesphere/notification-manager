...
func NewRuntime(kubeConfig string, stopCh <-chan struct{}) (*Runtime, error) {
	...
	r := &Runtime{
		kubernetesClient,
		informerFactory,
		rbac.NewRBACAuthorizer(am.NewOperator(kubernetesClient.KubeSphere(), kubernetesClient.Kubernetes(), informerFactory)),
...
func (r *Runtime) waitForResourceSync() error {
	...
	go func() {
		_ = runtimeCache.Start(r.stopCh)
	}()
	runtimeCache.WaitForCacheSync(r.stopCh)
